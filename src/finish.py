...
class Finish(Tk): # вывод результатов работы программы
    ...
    def results(self): # вывод на экран результатов работы программы (правильность ввода)
        ...
        self.lab4.config(text='', bg='#AFEEEE',font=('Arial', 13, 'bold'))  # настройка внешнего вида
        self.lab4.grid(row=3, column=0, columnspan=2)  # зафиксировали на окне
        if self.badly:
            for i in self.badly:
                tx2.insert(END, i+'\n') # добавили в конец
        else:
            self.lab4.config(text='Вы правильно ввели все слова', fg='green')  # настройка внешнего вида
        if self.good:
            for j in self.good:
                tx1.insert(END, j+'\n') # добавили в конец
        else:
            self.lab4.config(text='Вы не ввели ни одного правильного слова', fg='red') # настройка внешнего вида
