...
class Program(Tk):
    ...
    def __init__(self, number, lines):  # ввод переменных с которыми будем работать
        ...
        self.next = Button(self, text='Следующее слово', command=self.next_word)  # кнопка
        self.next.config(width=25)
        self.next.grid(row=2, column=0, sticky='e')  # зафиксировали на окне
        self.help_word = Button(self, text='Подсказка', command=self.help)  # кнопка
        self.help_word.config(width=25)
        self.help_word.grid(row=2, column=1, sticky='w')  # зафиксировали на окне
    ...
    def next_word(self):
        ...
        if error(word):
            ...
            if checking(word, self.words):
                self.input = str(self.words[0])+'-'+str(self.words[1])  # слово в список прав
                self.good.append(self.input)  # добавляем в конец массива переменную
            else:
                self.input = str(self.words[0]) + '-' + str(self.words[1])  # слово в список прав
                self.badly.append(self.input)  # добавляем в конец массива переменную
...
def checking(word, words):
    """определения правильности введённого слова"""
    if word == words[1]:
        return True
    return False
...
def error(word):
    """блокирует введённые цифры и тд."""
